...
export default function OriginSelector({ onOrigin }: OriginSelectorProps) {
  ...
  useEffect(() => {
    if (onOrigin) {
      onOrigin(selected?.id)
    }
  }, [selected])

  return (
    <div className="select" onFocus={handleInput} onBlur={handleClose}>
      <div className="select-custom">
        <h3>{t('originAddress')}</h3>
